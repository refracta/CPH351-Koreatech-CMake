...
int main(void) {
    ...
    timer.setTimerName(1, (char *) "Version 1-1\t");
    timer.setTimerName(2, (char *) "Version 1-2\t");
    timer.setTimerName(3, (char *) "Version 2-1\t");
    timer.setTimerName(4, (char *) "Version 2-2\t");
    timer.setTimerName(5, (char *) "Version 3-1\t");
    timer.setTimerName(6, (char *) "Version 3-2\t");
    timer.initTimers();

    float *data = new float[NUM_ELE];

#pragma omp parallel for num_threads(NUM_THREADS)
    LOOP_I(NUM_ELE) data[i] = getRandNum();

    //*** serial
    int bin_serial[NUM_BINS] = {0};

    timer.onTimer(0);
    LOOP_I(NUM_ELE) bin_serial[(int) data[i]]++;
    timer.offTimer(0);
...
#pragma omp parallel for num_threads(NUM_THREADS)
    ...
    LOOP_I(NUM_BINS) omp_init_lock(&locks[i]);
...
#pragma omp for
        LOOP_I(NUM_THREADS) LOOP_J(NUM_BINS) {
#pragma omp atomic
...
#pragma omp for
        LOOP_I(NUM_BINS) {
            LOOP_J(NUM_THREADS) bin_v2_2[i] += localBins[j][i];
        }
    }
...
#pragma omp barrier
    ...
    for (int i = 1; i < NUM_BINS; i += 2) {    // collect resutls of odd threads to master
...
void printResults(int *_bins) {
    LOOP_I(NUM_BINS) printf("%d ", _bins[i]);
    printf("\n");
}