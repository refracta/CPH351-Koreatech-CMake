...
int main(int argc, char *argv[]) {
    ...
    timer.setTimerName(1, (char *) "Kernel - basic");
    timer.setTimerName(2, (char *) "Kernel - shared memory");
    timer.setTimerName(3, (char *) "Kernel - no bank conflict");
...
#pragma omp parallel for num_threads(4)
    for (int row = 0; row < m; row++) {
        for (int col = 0; col < n; col++) {
            ...
            Ccpu[cIndex] = 0;
            LOOP_I(k) Ccpu[cIndex] += (A[ID2INDEX(row, i, k)] * B[ID2INDEX(i, col, n)]);
        }
    }
    printf("CPU finished!\n");
    timer.offTimer(0);
...
__global__ void MatMul(float *matA, float *matB, float *matC, int m, int n, int k) {
    ...
    LOOP_I(k) val += KERNEL_MUL(matA[ID2INDEX(row, i, k)], matB[ID2INDEX(i, col, n)]);
